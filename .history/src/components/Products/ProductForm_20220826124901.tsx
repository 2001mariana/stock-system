...
function ProductForm({ onSubmit, formProps, onUpdate }: ProductFormProps) {
  ...
  const disabledButtonControl = (
    labelButton: string,
    formProps: Product | undefined
  ) => {
    if (
      ...
      return true
    } else if (labelButton === 'Update' && formProps) {
      if (
        (form.name === String(formProps.name),
        form.price === parseFloat(formProps.price),
        form.stock === Number(formProps.stock))
      ) {
        return true
      }
    } else {
  ...
  return (
    <Form title="Product Form" onSubmit={handleFormSubmit}>
      ...
      <Button
        ...
        color={'primary'}
        variant={'solid'}
        disabled={disabledButtonControl(labelButton, formProps)}
      />
    </Form>
  )
}
